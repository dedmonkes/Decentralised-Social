...
use multigraph::{cpi::accounts::{CreateEdge as CreateEdgeGraph, CreateNode}, EdgeRelation, ConnectionType};

use crate::{state::{OwnerRecord, Identifier, Identity}, id};
...
pub fn create_edge(ctx: Context<CreateEdge>, connection_type : ConnectionType, edge_direction : EdgeRelation) -> Result<()> {
    ...
    let address = Pubkey::create_program_address(&seeds, &id()).unwrap();
    msg!("{:?} {:?}", identity_key, address);
